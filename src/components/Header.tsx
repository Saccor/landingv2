'use client';

import Image from 'next/image';
import Link from 'next/link';

const Header = () => {
  return (
    <header className="
      w-full bg-black relative z-50
      
      /* Mobile: Compact header (0-767px) */
      h-[51px]
      
      /* Tablet: Medium header (768px-1023px) */
      md:h-[87px]
      
      /* Desktop: Full header (1024px+) */
      lg:h-20
    ">
      {/* Mobile & Tablet: Logo perfectly centered with floating Survey link */}
      <div className="lg:hidden absolute inset-0 flex items-center justify-center px-4 md:px-16">
        {/* Perfectly Centered Logo */}
        <Link href="/" className="flex items-center justify-center">
          <svg 
            width="77" 
            height="27" 
            viewBox="0 0 78 28" 
            fill="none" 
            xmlns="http://www.w3.org/2000/svg" 
            className="
              /* Mobile: Standard size (0-767px) */
              w-[77px] h-[27px]
              
              /* Tablet: Slightly larger (768px-1023px) */
              md:w-[85px] md:h-[30px]
            "
          >
            <path d="M56.4101 7.48417L56.1825 8.14693L50.9273 23.4307L45.6686 8.12203C45.5372 7.74026 45.1817 7.48417 44.7816 7.48417H37.7148V6.40407C37.7148 5.28722 37.9565 4.50827 38.4399 4.06604C38.9233 3.60128 39.7634 3.36771 40.9602 3.36771V0.0527344C38.474 0.0527344 36.6436 0.564919 35.4703 1.5881C34.3193 2.58876 33.7444 4.1929 33.7444 6.40289V7.48298H31.4999C30.1882 7.48298 29.0489 7.7284 28.0821 8.28683C27.1388 8.82154 26.3785 9.57796 25.8036 10.5549V7.48298H21.8672V27.0527H25.8036V16.8695C25.8036 14.9156 26.1955 13.5427 26.978 12.7519C27.7841 11.961 28.9574 11.3054 30.5003 11.3054H33.7456V27.0516H37.716V11.3054H42.0114C42.401 11.3054 42.7506 11.5496 42.8891 11.9172L48.4857 26.8275L48.5701 27.0516H53.2176L53.3173 26.7883L60.3148 8.23703L60.5893 7.48298H56.4113L56.4101 7.48417Z" fill="white"/>
            <path d="M77.9813 16.8116C77.9813 14.9786 77.5964 13.3638 76.8256 11.9672C76.0547 10.5468 74.9777 9.44776 73.5955 8.66881C72.2134 7.88986 70.6376 7.50098 68.8695 7.50098C67.1013 7.50098 65.4012 7.90172 63.9734 8.70319C62.5677 9.50467 61.4684 10.65 60.6752 12.1391C59.9044 13.6057 59.5195 15.3118 59.5195 17.2586C59.5195 19.2054 59.9161 20.9233 60.7093 22.4125C61.5259 23.8779 62.6358 25.0244 64.0414 25.8484C65.4693 26.6498 67.0791 27.0506 68.8695 27.0506C71.0682 27.0506 72.9045 26.5005 74.3781 25.4014C75.8741 24.2786 76.9054 22.8476 77.4721 21.1059H73.2893C72.4281 22.8701 70.9544 23.7522 68.8683 23.7522C67.4181 23.7522 66.1932 23.2946 65.1959 22.3781C64.1986 21.4616 63.6437 20.2475 63.5298 18.7359H77.8452C77.9355 18.1632 77.9813 17.5218 77.9813 16.8116ZM63.5639 15.575C63.7457 14.1084 64.3124 12.9524 65.264 12.1047C66.2155 11.2345 67.3606 10.7994 68.6982 10.7994C70.1718 10.7994 71.4073 11.2345 72.4046 12.1047C73.4019 12.975 73.9123 14.1321 73.9346 15.575H63.5639Z" fill="white"/>
            <path d="M16.3154 7.49065V10.5484C15.6584 9.67811 14.7737 8.95607 13.6626 8.38342C12.5749 7.78824 11.2937 7.48947 9.82005 7.48947C8.188 7.48947 6.69205 7.90206 5.33221 8.72606C3.99466 9.52754 2.92931 10.6622 2.13616 12.1276C1.36531 13.5942 0.980469 15.2778 0.980469 17.1783C0.980469 19.0789 1.36531 20.7861 2.13616 22.2978C2.92931 23.7869 3.99466 24.9548 5.33221 25.8025C6.66976 26.6277 8.14342 27.0391 9.75317 27.0391C11.2045 27.0391 12.4963 26.7415 13.6297 26.1463C14.7631 25.5511 15.6584 24.8066 16.3154 23.9126V27.0391H20.226V7.48828H16.3154V7.49065ZM15.4988 20.6854C14.9778 21.6469 14.2856 22.3808 13.4244 22.8847C12.5632 23.3886 11.634 23.6411 10.6367 23.6411C9.63936 23.6411 8.73241 23.3897 7.84892 22.8847C6.98772 22.3583 6.28492 21.6018 5.74052 20.6166C5.21957 19.6313 4.95793 18.486 4.95793 17.1807C4.95793 15.8753 5.2184 14.7525 5.74052 13.8135C6.28492 12.852 6.98772 12.13 7.84892 11.6486C8.71012 11.1447 9.63936 10.8922 10.6367 10.8922C11.634 10.8922 12.5632 11.1435 13.4244 11.6486C14.2856 12.1525 14.9767 12.8852 15.4988 13.8479C16.0432 14.8095 16.3154 15.9441 16.3154 17.2494C16.3154 18.5548 16.0432 19.7001 15.4988 20.6854Z" fill="white"/>
          </svg>
        </Link>
      </div>

      {/* Survey Link - Floating on right side */}
      <div className="
        lg:hidden absolute top-0 h-full flex items-center
        
        /* Mobile: Standard padding (0-767px) */
        right-4
        
        /* Tablet: More padding (768px-1023px) */
        md:right-16
      ">
        <Link 
          href="/survey" 
          className="
            text-white font-medium hover:underline transition-colors
            
            /* Mobile: Smaller text (0-767px) */
            text-sm
            
            /* Tablet: Standard text (768px-1023px) */
            md:text-base
          "
        >
          Survey
        </Link>
      </div>

      {/* Desktop: Constrained container layout */}
      <div className="hidden lg:flex max-w-[1440px] mx-auto px-4 lg:px-32 h-full items-center justify-between">
        {/* Left: Empty for symmetry or future nav */}
        <div className="w-1/3" />

        {/* Center: Logo */}
        <div className="flex justify-center items-center w-1/3 h-full">
          <Link href="/" className="relative w-full h-full lg:w-28 lg:h-10 flex items-center justify-center flex-shrink-0">
            {/* Desktop Logo - Show on screens 1024px and larger */}
            <Image
              src="/logo.svg"
              alt="Logo"
              width={120}
              height={42}
              className="object-contain"
              priority
            />
          </Link>
        </div>

        {/* Right: Navigation buttons - Desktop only */}
        <div className="w-1/3 hidden lg:flex justify-end items-center gap-4">
          <Link href="/survey" className="text-white text-sm font-medium hover:underline transition">
            Survey
          </Link>
        </div>
      </div>
    </header>
  );
};

export default Header;